...
```json
{
  ...
  "epicit": [
    ...
    {
      "id": "aikajana",
      "nimi": "🗓️ Timeline & Review — Aikajana ja jälkiarviointi",
      "kuvaus": "Otteluohjelma kaikille seuratuille sarjoille, raahattava aikajana, kumulatiivinen kerroinhistoria ja kierrosarviointi joka erottaa analyysivirheen epäonnesta.",
      "tiketit": [74, 75, 76, 77, 78, 79, 80, 81, 82, 83, 84, 85, 86, 87, 88],
      "user_story": "Vedonlyöjänä haluan selata aikajanalla eteen ja taakse nähdäkseni mitä on tulossa ja mitä meni ohi, ja haluan menneistä kierroksista arvion jossa kerrotaan meninkö analyysissä pieleen vai kävikö vain huono tuuri — jotta tiedän milloin mallia pitää korjata ja milloin ei.",
      "valmius": 100
    }
  ],
  "tiketit": [
    ...
    { "id": 74, "epic": "aikajana", "nimi": "Otteluohjelmakalenteri (fixtures.json)", "status": "done", "acceptance_criteria": ["Koko aikaikkuna (-21…+35 pv) kaikille seuratuille sarjoille: 416 ottelua, 38 päivää, 8 sarjaa", "LÄHDE ESPN EIKÄ ODDS API — kalenterin hakeminen kertoimien kautta polttaisi kuukauden kvootan päivissä (sama laskuvirhe kuin #65)", "Kertoimet LIITETÄÄN normalisoiduilla nimillä + päivällä, ei ID-vertailuna: ESPN ja Odds API nimeävät joukkueet eri tavoin", "Päivälistaan vain päivät joilla on otteluita — tyhjää päivää ei listata", "ESPN:n fin.1 palauttaa 200 + tyhjän listan vaikka Veikkausliigaa pelataan, joten kalenteri on ESPN-ohjelma ∪ tunnetut kerroinkohteet", "22 yksikkötestiä"], "valmius": 100 },
    { "id": 75, "epic": "aikajana", "nimi": "Kerroinhistoria (odds-history.json)", "status": "done", "acceptance_criteria": ["history/*.json käännetään ottelukeskeiseksi aikasarjaksi: yksi rivi per ottelu, sisällä havaintopisteet", "KUMULATIIVINEN: uusi ajo liittää havainnot vanhojen perään, koska history/-hakemistoa siivotaan mutta kerroinhistoria on juuri se osa joka halutaan säilyttää", "Duplikaatit tunnistetaan aikaleimasta — sama ajo kahdesti ei kasvata sarjaa", "Valioliigan avauskierros tallessa OIKEISTA havainnoista: 10 ottelua, 6 kerroinhavaintoa kustakin. Mitään ei rekonstruoitu muistista", "12 yksikkötestiä"], "valmius": 100 },
    { "id": 76, "epic": "aikajana", "nimi": "Kierrosarviointi (reviews.json)", "status": "done", "acceptance_criteria": ["Maaliaikajana erottaa analyysivirheen epäonnesta: \"johti 83 min ja kaatui\" vs \"ei ollut voitolla kertaakaan\"", "Oikeasta datasta: Coventry @21.00 EI KOSKAAN voitolla (0 min), Newcastle @4.00 johti loppuun asti (83 min)", "RAJAUS SANOTAAN: jalkapallossa mikään tulos ei ole mahdoton ennen loppuvihellystä, joten tämä ei väitä mittaavansa \"mahdollisuutta\" vaan sitä kuinka kauan kohde oli VOITOLLA", "Ilman aikajanaa verdikti on ei_tietoa eikä oli_voitolla — puuttuvaa tietoa ei esitetä havaintona", "KORJAUS: clock.value on SEKUNTEJA (874 = 15. min) — ilman muunnosta maali olisi merkitty 874. minuutille", "KORJAUS: template literalissa \\\\s ja \\\\d eivät ole regex-escapeja; JS syö kenoviivan. Toimi vain team.id-varareitin varassa", "Oma maali luetaan tekstin pistetilanteesta, koska team.id osoittaa tekijään eikä siihen jonka hyväksi maali meni", "38 yksikkötestiä"], "valmius": 100 },
    { "id": 77, "epic": "aikajana", "nimi": "KORJAUS: otsikko lupasi value-kohteita joita korteissa ei ollut", "status": "done", "acceptance_criteria": ["Otsikkolaskuri käytti raakaa edgeä (> 0.03), kortit palvelimen flag-kenttää — eri asia", "Palvelin voi kieltäytyä liputtamasta kohdetta jonka edge ylittää kynnyksen, jos mallin luottamus ei riitä (tiketti #53)", "Arkistossa tällaisia oli TASAN 3 — sama luku jonka käyttäjä näki", "flaggedEdges()/hasFlag() yhtenä totuutena; kynnyksen paikka on palvelimella, selain lukee päätöksen", "3 E2E-regressiotestiä"], "valmius": 100 },
    { "id": 78, "epic": "aikajana", "nimi": "Simulaatio Admin-togglen taakse", "status": "done", "acceptance_criteria": ["Oletuksena piilossa: ohjelma käyttää oikeaa dataa, ja simuloitu tulos näyttää kortilla samalta kuin pelattu", "EI POISTETTU — testattu (#32) ja hyödyllinen kun otteluita ei ole käynnissä", "19 E2E-testiä hajosi KELLOSTA EI KOODISTA: today.json:in ainoa ottelu oli eilinen, joten kortti piirtyi historiatilassa ilman nappeja", "Korjaus: useFixtureSnapshot niihin spekkeihin jotka tarvitsevat interaktiivisen ottelun — apuri oli olemassa juuri tätä varten", "Samalla löytyi että request.get ei kulje page.routen läpi, joten kaksi testiä vertasi elävää tiedostoa fikstuuriin", "5 uutta E2E-testiä"], "valmius": 100 },
    { "id": 79, "epic": "aikajana", "nimi": "Aikajanakontrolli — raahattava, mobiili huomioitu", "status": "done", "acceptance_criteria": ["Aikajana korvaa kiinteät päivänapit ja kattaa koko aikaikkunan", "TYHJIÄ PÄIVIÄ EI NÄYTETÄ — käyttäjä selaa kierroksesta kierrokseen", "KERTOIMETON OTTELU NÄKYY otteluohjelmana, ei piiloteta: muuten käyttäjä luulisi ettei ottelua ole", "Raahaus pointer-eventeillä; vaikea osa on ETTEI KLIKKAUS HUKU — 6 px liikekynnys erottaa ne", "Scrollbar teeman mukainen; kosketuslaitteella piilotettu kokonaan (6 px on liian pieni sormelle)", "Nauha vierittyy valittuun päivään — muuten se jäi kolmen viikon takaisiin päiviin", "Kalenterin puuttuminen EI kaada mitään: paluu vanhaan navigointiin, puute sanotaan", "SEURAUSKORJAUS: ennakkohakunappi käytti siirtymää, aikajanan valinta on absoluuttinen päivä — nappi ei olisi ilmestynyt koskaan", "11 yksikkötestiä + 15 E2E-testiä"], "valmius": 100 },
    { "id": 80, "epic": "aikajana", "nimi": "Menneet kierrokset Historia-välilehdelle", "status": "done", "acceptance_criteria": ["Kierros kerrallaan mitä malli sanoi ja mitä tapahtui, maaliaikajana palkkina", "Analyysivirheet punaisena varoituksena kierroksen otsikkotasolle — ei tarvitse etsiä otteluiden seasta", "Malli ja markkina aina rinnakkain, alle 20 ottelun otos varoituslaatikkona", "10 E2E-testiä"], "valmius": 100 },
    { "id": 81, "epic": "aikajana", "nimi": "Aikajana ilman selainpalkkia", "status": "done", "acceptance_criteria": ["Palkki piilotettu kaikilta selaimilta (scrollbar-width, -webkit-scrollbar, -ms-overflow-style)", "POISTO EI SAA OLLA HUONONNUS: palkki teki kaksi asiaa, molemmat korvattiin", "Reunahäivytys korvasi vihjeen että sisältöä on lisää — näkyy vain siihen suuntaan johon voi liikkua", "Hiiren pystyrulla ohjattiin vaakasuuntaan; preventDefault vain kun rullaus todella kuluu nauhaan, muuten sivu jumittuisi", "Sormella natiivi swipe (touch-action: pan-x pan-y) — selaimen oma vieritys antaa inertian ilmaiseksi", "user-select: none — raahaus maalasi chippien tekstin", "KORVATTU tiketissä #82"], "valmius": 100 },
    { "id": 82, "epic": "aikajana", "nimi": "Päivänavigointi viitenä nappina", "status": "done", "acceptance_criteria": ["Vieritettävä nauha oli huono käyttää: piti raahata oikeaan kohtaan, päivät vilisivät, ei nähnyt missä ollaan", "Tilalla ‹ · Eilen · Tänään · Huomenna · › — mahtuu 320 px ruudulle, ei vieritystä", "NUOLET HYPPÄÄVÄT OTTELUPÄIVIIN eivät kalenteripäiviin: sokea +1 vrk veisi tyhjälle päivälle", "Nuoli himmenee kun ottelupäiviä ei ole enää — ei vie tyhjään näkymään", "Päivä jolla ei pelata jää himmeäksi mutta PYSYY painettavana", "Valittu päivä sanotaan erikseen kun se ei ole mikään kolmesta pikavalinnasta", "\"Kaikki\" poistui napista mutta haara jäi: aiemmin sen painanut on se localStoragessaan, ja E2E-fikstuuri käyttää sitä suojana kellosta riippuvalta häilyvyydeltä", "SEURAUSKORJAUS: ennakkohakunappi käytti siirtymää, valinta on absoluuttinen päivä — nappi ei olisi ilmestynyt koskaan", "Poistettu orpona: raahaus, rullaohjaus, reunahäivytys, updateEdges, scrollToActive, attach, legacyDayNav", "13 yksikkötestiä + 19 E2E-testiä"], "valmius": 100 },
    { "id": 83, "epic": "aikajana", "nimi": "KORJAUS: kaksi arkistoa jotka eivät puhuneet keskenään", "status": "done", "acceptance_criteria": ["Pelattu ottelu jolle oli kertoimet, tunnusluvut ja mallin arvio näkyi \"ilman julkaistuja kertoimia\" -listassa", "Selaimen bt_odds_archive (#60) ja palvelimen odds-history.json (#75) olivat erillään; kortit lukivat vain ensimmäistä", "Tyhjällä selaimella palvelin tiesi ja selain ei — ja ottelu oli jo pudonnut today.jsonista", "football-server-archive.js lukee palvelinarkiston; päivänäkymä yhdistää kolme lähdettä: palvelin → selain → snapshot", "odds-history.json tallettaa avaushavainnon toimistorivit, tunnusluvut ja edge-rivit KERRAN per ottelu (67 KB → 139 KB)", "Tekstit rehellisiksi: \"N ottelua ilman kertoimia\" väitti aina samaa", "useIsolatedArchives-apuri: kolme datalähdettä tarkoittaa että testin pitää ohjata kaikki kolme", "Poistettu kaksi ehdollista skippausta jotka piilottivat tämän", "11 E2E-testiä"], "valmius": 100 },
    { "id": 84, "epic": "aikajana", "nimi": "Ratkennut ottelu näyttää lopputuloksen", "status": "done", "acceptance_criteria": ["Kortti sanoi \"alkanut\" ottelusta joka oli pelattu loppuun tunteja sitten — teknisesti tosi, mutta ei kerro mitä tapahtui", "Nyt \"2–3 · ratkennut\"", "Tulos palvelinarkistosta (ensisijainen, jo tulkittu 1X2:ksi) tai kalenterista", "calendarMatch() etsii TUNNISTEELLA eikä päivällä: kalenterin date on UTC-johdettu, korttien päiväavain paikallinen", "Regressiotesti: \"ratkennut\" ei saa vuotaa alkamattomiin otteluihin"], "valmius": 100 },
    { "id": 85, "epic": "aikajana", "nimi": "Veikkausliigan otteluohjelma Palloliiton tulospalvelusta", "status": "done", "acceptance_criteria": ["ESPN:n fin.1 palauttaa HTTP 200 ja TYHJÄN listan vaikka Veikkausliigaa pelataan — sarja katosi kalenterista", "Lähde on Palloliiton oma tulospalvelu (Torneopal): virallinen ja auktoritatiivinen", "robots.txt tarkistettu 25.8.2026: sallii kaiken paitsi /person/", "Rajapinta kaivettiin SPA:n bundlesta; kutsu vaatii kolme asiaa: accept-avain, kyselytunniste ja Origin — ilman Originia 403", "Löytötapa dokumentoitu tiedostoon, jottei seuraava ylläpitäjä joudu arvaamaan", "competition_id johdetaan vuodesta (spljp26), ei kovakoodata — muuten kausi jää menneeseen", "OMA BUGI: aikavyöhykemuunnos vertasi Helsinki-renderöintiä raakaan UTC-lukuun; toimii vain UTC-koneella, Suomessa siirtymäksi mitattiin nolla ja illan ottelut jäivät väärään tuntiin", "Veikkausliiga kalenterissa 0 → 24 ottelua; koko kausi 132 (126 pelattua + 6 julkaistua tulevaa)", "Haun pettäminen degradoituu tyhjään listaan, ei kaada kalenteria", "19 yksikkötestiä"], "valmius": 100 },
    { "id": 86, "epic": "aikajana", "nimi": "Blend-paino 0.35 → 0.40 mitattuna", "status": "done", "acceptance_criteria": ["Arvo EI ole arvattu: calibrateBlendWeight() minimoi Brier-scoren toteutuneista tuloksista", "Kolme ehtoa täyttyi: otos 21 ≥ 20, optimi hakuvälin SISÄLTÄ, muutos pieni (0.05)", "Edellisellä ajolla (n=20) sama laskenta ehdotti painoa 1.0 REUNALTA — sitä ei sovellettu juuri siksi", "Peruste kirjattu config.ts:ään, jotta seuraava säätö tehdään samoilla ehdoilla"], "valmius": 100 },
    { "id": 87, "epic": "aikajana", "nimi": "Lähdebanneri huomioi katsottavan päivän", "status": "done", "acceptance_criteria": ["\"Kertoimet haettu 365 min sitten — aja putki uudelleen\" näkyi myös mennyttä kierrosta katsottaessa", "Vanhentuneisuus on suhteellinen KATSOTTAVAAN PÄIVÄÄN eikä kelloon: menneellä päivällä kertoimien kuuluu olla vanhoja", "Putken ajaminen ei toisi takaisin eilisiä hintoja, joten ohje oli väärä", "Menneellä päivällä: \"📜 Menneen päivän kohteet · ARKISTO · vetoa ei voi enää lyödä\"", "3 E2E-testiä"], "valmius": 100 },
    { "id": 88, "epic": "aikajana", "nimi": "Kerroinruudun taustaväri jokaiselle hinnalle", "status": "done", "acceptance_criteria": ["Väri piirtyi vain ⭐-ruutuun, koska snapshot laskee edgen pelkästä parhaasta hinnasta: 10.00 oli värillinen ja saman kohteen 9.80 musta — vaikka jos 10.00 on ylikerroin, niin on 9.80 myös", "Nyt jokainen ruutu saa oman edgensä omasta hinnastaan: model_prob × tehollinen kerroin − 1, pörssin komissio mukaan luettuna (Betfair 9.80 → +4.5 %, kirjan 9.70 → +8.3 %)", "Neljä tasoa samoilla kynnyksillä kuin jääkiekkokortilla: musta, keltainen > 3 %, vihreä > 5 %, kultainen > 8 %", "Odotusarvoprosentti näkyy ruudussa kertoimen alla — väri ilman lukua jättäisi käyttäjän arvaamaan miksi ruutu on keltainen", "VÄRI JA PANOSSUOSITUS OVAT ERI ASIOITA: lippu vaatii edgen lisäksi 2 pp:n eron mallin ja markkinan välillä (tiketti #53), joten kultainen ruutu voi olla ilman suositusta. Vihjeteksti sanoo kumpi tilanne on käsillä — tiketin #77 opetus säilyy, ero SELITETÄÄN eikä piiloteta", "KORJAUS: kortti väitti \"+11.6 % — jää alle 3 %:n kynnyksen\", mikä on suoraan väärin. Nyt syy on oikea: edge tulee pitkästä maksusta, todennäköisyysero vain 0.7 pp", "Kynnykset lukittu yksikkötestillä palvelimen VALUE_THRESHOLD / STRONG_THRESHOLD / MIN_PROB_EDGE:hen — kolmessa paikassa asuva kynnys eriytyy muuten hiljaa", "Vanha E2E-testi vertasi värillisten ruutujen määrää lippujen määrään eli lukitsi juuri väärän säännön; tilalla ruutukohtainen odotusarvo", "13 yksikkötestiä + 2 E2E-testiä, 821 unit ja 207 e2e vihreänä"], "valmius": 100 },
    {"id": 103, "epic": "aikajana", "nimi": "Liigan avauskierros: ennakko dokumentista, lähtö-Elo kortille, Veikkaus käsin", "status": "done", "acceptance_criteria": ["JUURISYY LÖYTYI: tyhjä kausi ei ole sama kuin puuttuva tilastolähde. liiga.fi palautti kauden 2027 ohjelman ilman yhtäkään päättynyttä ottelua, joten `current.teams` oli TYHJÄ LISTA — mutta koodi tarkisti vain `if (!stats)`, eikä tyhjä kausi ollut null. Kausiennakon varareitti ei ajanut, strengthForTeam palautti nullin ja kortti päätyi market-only-tilaan ILMAN tunnuslukuja. Siksi lähtö-Elo ei näkynyt joukkueiden nimissä, vaikka Elo-kartta oli laskettu oikein", "Vika oli hiljainen juuri siksi että kaikki näytti toimivan: lähdelistassa luki liiga.fi, kertoimet olivat kohdillaan, lokissa ei ollut virhettä", "KAUSIENNAKKO ON NYT DOKUMENTTI EIKÄ KOODI: `data/liiga-kausiennakko-2026-27.md` on lähde, `liiga-preview.ts` jäsentää sen. Lähtö-Elo luetaan taulukon sarakkeesta EIKÄ johdeta sijasta (eloFromRank), jotta dokumentissa ja kortilla on sama luku. Tiedosto sanoi tämän puutteen itse: 'Korjattava huomiselle: pipeline lukee prioria vielä suoraan koodista'", "TEAM_PRIORS jää varalle: puuttuva tai rikkinäinen dokumentti ei kaada putkea. Osittain jäsentynyt taulukko HYLÄTÄÄN kokonaan (MIN_PREVIEW_TEAMS) — puolikas taulukko antaisi osalle joukkueista lähtö-Elon ja osalle ei, ja ero näkyisi kortilla mielivaltaisena", "Sulkeet huomioiva pilkkojako: 'terävä kärki (Blichfeld, Rautiainen)' on yksi kohta eikä kaksi joista toinen olisi 'Rautiainen)'", "ENNAKON PLUSSAT JA MIINUKSET KORTILLE rakenteisena (`MatchCard.preview`). Ne olivat snapshotissa jo ennestään mutta VAIN mallin perustelutekstissä yhtenä pilkkuluettelona Analyysi-osion pohjalla. Nyt kärjet näkyvät ilman osion avaamista ja koko lista 📋 Ennakko -osiossa. Tekstimuoto säilyy adjustments-kentässä koska LLM-kysely ja laskentaerittely lukevat sitä", "LÄHDE SANOTAAN KORTILLA: ennakko on yhden toimituksen arvio eikä mittaus, ja käyttäjän pitää nähdä kenen arvio se on ennen kuin punnitsee sen", "VEIKKAUKSEN KERTOIMET KÄSIN (`data/veikkaus-odds-manual.json`): The Odds API ei kata Veikkausta eikä Veikkaus tarjoa julkista kerroinrajapintaa, joten kerroinlista vertaili hintoja joita suomalainen käyttäjä ei voi pelata. Scrapaus olisi hauras ja kyseenalainen; käsisyöttö on rehellinen ja näkyy sellaisena myös kortilla (✍️)", "KÄSIN SYÖTETTY OSALLISTUU LASKENTAAN, ei ole näytös: paras hinta, kate, markkinamediaani, edge ja Kelly lasketaan se mukana. Rivi lisätään ENNEN kortin rakennusta — jos se työnnettäisiin valmiiseen korttiin, lista näyttäisi hinnan jota analyysi ei tunne", "Kolme turvasääntöä: rivi lisätään vain jos ottelu täsmää (sarja + päivä + molemmat joukkueet), rajapinnasta tullut sama toimisto voittaa aina (ei kahdesti mediaaniin), ja täsmäämättä jäänyt rivi RAPORTOIDAAN — hiljainen ohitus näyttää samalta kuin 'kertoimia ei ollut'", "Veikkauksen 'K-Espoo' ja liiga.fi:n 'Kiekko-Espoo' ovat sama joukkue (LIIGA_ALIASES). Ilman tätä yksi seitsemästä ottelusta olisi jäänyt ilman kertoimia huomaamatta — sama vika kuin eloKeyFor():n kanssa tiketissä #40", "Linkki vie Veikkauksen kerroinsivulle. EI ↗-merkkiä: syvälinkkiä yksittäiseen otteluun ei ole ilman rajapintaa, ja väärin luvattu syvälinkki olisi pahempi kuin rehellinen etusivulinkki", "`npm run odds:manual` cron-vaiheeksi: idempotentti ja ILMAINEN, joten käsisyötetyn tiedoston päivitys ei vaadi kerroinhaun ajamista uudelleen (se maksaa kvoottaa). Kortit lasketaan uudelleen jotta hinta on mukana kaikessa mitä siitä johdetaan", "KORJAUS: lähdelista väitti Liigan Elon tulevan ESPN:stä. Se tulee kausiennakosta — väärä lähdemerkintä on pahempi kuin puuttuva, koska koko listan tarkoitus on että luvun voi jäljittää", "49 uutta yksikkötestiä (liiga-preview, odds-manual) + 10 regressiotestiä tyhjälle kaudelle + 7 E2E-testiä omalla Liiga-fikstuurilla. 1012 unit ja 201 e2e vihreänä (13 e2e-epäonnistumista on ennestään olemassa eikä liity tähän)"], "valmius": 100}
  ],
