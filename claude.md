...
```json
{
  ...
  "tiketit": [
    ...
    { "id": 29, "epic": "jalkapallo", "nimi": "Jalkapallouutiset per ottelu", "effort": "M", "riippuvuudet": [23], "status": "todo", "acceptance_criteria": ["Jalkapallo-RSS-syötteet (BBC, Guardian, Yle, IS)", "Joukkuenimien matchaus otteluihin", "LLM-erittely jalkapalloprompilla (nlp.ts uudelleenkäyttö)", "0-5 uutista per ottelu confidencella ja linkeillä", "Duplikaatit estetty source_url:n perusteella"], "valmius": 0 },
    ...
    { "id": 32, "epic": "jalkapallo", "nimi": "Jalkapallon päiväsimulaatio", "effort": "L", "riippuvuudet": [26, 30], "status": "todo", "acceptance_criteria": ["Maalit Poisson-mallin lambda-arvoista (ka ~2.7 maalia/ottelu)", "90 min pakattuna ~20 sekuntiin, puoliaika, lisäajat", "Maalintekijät, keltaiset/punaiset kortit, kulmat, kumuloituva xG", "Ratkaisee 1X2 + O/U 2.5 + BTTS + pikavedon", "SIMULOITU-badge jokaiseen tulokseen", "Simuloidut (bt_sim_results) ja oikeat (bt_real_results) tulokset eri avaimissa"], "valmius": 0 },
    { "id": 33, "epic": "jalkapallo", "nimi": "Tarkkuus- ja CLV-mittarit", "effort": "S", "riippuvuudet": [32], "status": "todo", "acceptance_criteria": ["Osumatarkkuus, Brier score, ROI, CLV laskettu ja testattu", "Seuranta-välilehti näyttää mittarit", "Sim vs. oikeat tulokset eroteltu näkyvästi", "Blend-painon w kalibrointi Brier scoren perusteella"], "valmius": 0 },
    { "id": 34, "epic": "jalkapallo", "nimi": "GH Actions: snapshot-cron", "effort": "S", "riippuvuudet": [24, 25, 29], "status": "todo", "acceptance_criteria": ["Uusi workflow, workflow_dispatch tuottaa validin today.json:n", "Committoi public/data/ ja deployaa Pagesiin", "Cron 2x/vrk päälle kun secretit asetettu", "Lokitus: kvoottakäyttö ja jokaisen vaiheen onnistuminen"], "valmius": 0 }
  ],
